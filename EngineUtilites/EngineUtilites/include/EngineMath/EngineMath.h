...
namespace EngineUtilities { // Namespace general de la librería
  ...
  inline float fabs(float value) { return std::fabs(value); } // Alias de abs con nombre tradicional
  ...
  inline float EMax(float a, float b) { return std::max(a, b); }
  inline float EMin(float a, float b) { return std::min(a, b); }
